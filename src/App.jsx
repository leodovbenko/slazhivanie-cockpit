...
const SOL = {
  ...
  "Комьюнити": { icon: Users, color: c.clay, sub: "AI комьюнити-менеджер · закрытый чат-сообщество", cat: "Сообщество", soon: true,
    devStatus: "Вечерний дайджест «вопрос → как решилось» по реальным тредам ответов — уже на проде, и в чат он уходит только после подтверждения человека. Дальше по дорожной карте — ответы на вопросы по чату, недельный топ благодарностей и активности, модерация. Живой демо-дашборд появится здесь, когда агент выйдет в кокпит.",
    ...
    effect: "Меньше рутины", speed: "С нашим спецом",
    tags: ["Дайджест", "Знание", "Активность", "Модерация"],
    pitch: "Читает закрытый чат-сообщество с атрибуцией и каждый вечер собирает дайджест дня — какие вопросы задали и как их решили, тредами по реальным ответам-reply, а не выдуманным связям — и приносит его человеку на подтверждение. В чат пост уходит только после нажатия «Отправить»; не ответили — не уходит. В тихий день молчит. Знание сообщества держит отдельной полкой, не растворяя в памяти компании. Дальше — Q&A по чату, счёт благодарностей и активности, модерация. Промпт дайджеста редактирует оператор.",
    landing: {
