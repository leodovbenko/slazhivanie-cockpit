...
import {
  ...
  CircleDot, ArrowRight, Bot, Search, Check, Send, Link2, UserCheck, Lightbulb,
} from "lucide-react";
...
const SOL = {
  Argus: { icon: Satellite, color: c.violet, sub: "AI Chief of Staff", cat: "Руководителю",
    ...
    tags: ["Брифы", "Протоколы", "Задачи", "Память"],
    pitch: "Тихо слушает чаты и встречи и возвращает брифы к встречам, разборы 1:1, протоколы и находки. И ведёт список дел, который держит сам: что вы обещали и чего ждёте, у кого сейчас мяч. Растёт по грейдам — от писаря до right-hand уровня совета директоров.",
    landing: {
      situation: "У руководителя десятки чатов, встреч и голосовых в день. Всё важное проговаривается устно и в переписке — и там же тонет. А ещё десятки повисших дел: кому-то обещал, от кого-то ждёшь ответ — и половина забывается.",
      problem: "Договорённости теряются. К встрече готовиться некогда, а протокол после неё пишет тот, у кого дошли руки, — то есть чаще всего никто. Follow-up держится в голове: кому что должен, кто должен тебе, к какому сроку — пока не выпадет. Цели незаметно уходят из фокуса.",
      solution: "Аргус тихо слушает чаты и встречи. Возвращает бриф перед встречей, разбор 1:1, протокол сразу участникам и находки: забытые цели, риски, скрытые таланты. И ведёт список дел за вас: из встреч и чатов вытаскивает, что повисло, показывает, у кого мяч (ваш ход / ждёте ответ / просрочено), ставит дату возврата — предлагает, а закрываете вы. Растёт по грейдам — от писаря до правой руки.",
      benefit: "−34% времени руководителя на операционку, 95% задач с владельцем и сроком, follow-up больше не теряется — ни одно обещание и ожидание не выпадает. Меньше ручного контроля — больше видно.",
    } },
  ...
  "ИИ-Продакт": { icon: Lightbulb, color: c.clay, sub: "Развитие продукта из фидбэка", cat: "Руководителю",
    task: "Пожелания к продукту тонут в чатах — непонятно, услышали ли вас",
    result: "Ловит пожелание в чате, уточняет и возвращает статус. Ничего не теряется.",
    effect: "Больше контроля", speed: "В пару кликов",
    tags: ["Фидбэк", "Бэклог", "Приоритет", "Статус"],
    pitch: "Замечает в рабочих чатах, когда вы просите доработать продукт, задаёт один уточняющий вопрос и складывает пожелание в приоритезированный бэклог. Одинаковые просьбы разных команд склеиваются в «просят N» — это двигает приоритет. Взяли, выпустили или решили не делать — вам приходит статус в тот же чат. Так развивается и сама эта платформа.",
    landing: {
      situation: "Вы пользуетесь платформой и по ходу подкидываете идеи: «вот бы ещё…», «неудобно, что…». Пожелания рождаются прямо в рабочих чатах, между делом.",
      problem: "В обычной жизни такие пожелания тонут в переписке. Никто не помнит, кто что просил; вы не знаете, услышали ли вас и что с этим стало. Продукт тогда развивается по догадкам, а не по реальному спросу.",
      solution: "Первый агент замечает пожелание прямо в чате и задаёт один тёплый уточняющий вопрос, чтобы понять точно (или молчит, если и так ясно). Пожелание попадает в общий продуктовый бэклог; одинаковые просьбы разных команд склеиваются в «просят N клиентов» — сигнал приоритета. Когда берём в работу, выпускаем или решаем не делать — вам приходит короткий статус в тот же чат.",
      benefit: "Ни одно пожелание не теряется, и вы видите, что услышаны — и что именно поменялось благодаря вам. Продукт растёт из реального спроса; эта платформа развивается именно так. То же можно развернуть под ваш продукт — чтобы фидбэк вашей команды и клиентов превращался в приоритезированный бэклог (по запросу).",
    } },
  ...
  Hermes: { icon: Zap, color: c.cyan, sub: "Контент-машина · один человек — как отдел маркетинга", cat: "Маркетинг",
    task: "Нет времени на контент, голос бренда плывёт",
    result: "Один человек ведёт контент как целый отдел: ×10 публикаций без роста расходов.",
    effect: "Меньше рутины", speed: "В пару кликов",
    tags: ["LinkedIn", "Каналы", "Блог", "Voice"],
    pitch: "Один человек публикует как целый отдел маркетинга: стоимость единицы контента падает кратно. Сырая мысль на входе — пост в твоём голосе на выходе, по расписанию. Один повод раскладывает по всем каналам и публикует статьи в твой блог — после проверки бренда. Правила голоса само-обновляются из аналитики.",
    landing: {
      situation: "Ты знаешь, что регулярное присутствие работает на продажи, найм и репутацию — в LinkedIn, Telegram, на своём сайте. Но руки не доходят: писать часто, в своём голосе, да ещё под каждую площадку и попадать в то, что заходит аудитории.",
      problem: "Контент откладывается неделями. Когда всё же садишься писать — уходит час на один пост, а голос плывёт от настроения. Под каждый канал приходится переписывать вручную, статья в блог не выходит месяцами. Аналитику никто не смотрит, поэтому охват не растёт.",
      solution: "Кидаешь сырую мысль голосом или текстом в Telegram. Гермес пишет пост в твоём голосе, ты правишь в один тап — и он выходит по расписанию, дважды в будни. Один повод он раскладывает сразу по всем каналам — LinkedIn, Telegram, а статью «Блог сайта» публикует прямо на твой сайт после проверки бренда, с кнопкой «Снять». Блоги (в том числе на другом языке или домене) ты добавляешь и ведёшь сам. Правила голоса подтягивает из аналитики прошлых постов.",
      benefit: "Один человек ведёт контент как целый отдел маркетинга. Стоимость единицы контента падает кратно: минута на одобрение вместо часа на текст, один повод — сразу под все каналы и в блог, а не переписывание под каждый вручную. Значит, публиковать можно в разы больше, не раздувая расходы и не нанимая SMM-команду. В демо — 24 500 охвата и до 83 000 показов на топ-пост по MENA, 100% в твоём голосе. А где ты правишь его посты, платформа замечает разницу и на этом его подкручивает — со временем правок меньше.",
    } },
  ...
  Recruiter: { icon: UserCheck, color: c.blue, sub: "AI-агент-рекрутер", cat: "Найм и HR", soon: true,
    ...
    tags: ["hh.ru", "Светофор", "Калибровка", "Воронка"],
    pitch: "Читает полный текст резюме — не только поля — и раскладывает кандидатов по трём зонам светофора с обоснованием и цитатами. Критерии не выдумывает: применяет те, что задал рекрутер под конкретный наём. Калибровка сверяет его оценки с вашими решениями, аналитика воронки показывает узкое место, а разбор собеса идёт по компетенциям вакансии. Одно ядро настраивается под любую вакансию.",
    landing: {
      ...
      solution: "Агент читает полный текст каждого резюме и раскладывает по трём зонам светофора — с обоснованием и цитатами из резюме. Критерии не придумывает сам: применяет те, что рекрутер задал под конкретный наём. Передача нанимающему менеджеру — двусторонняя: он видит на отдельной странице, почему кандидат прошёл, и отвечает «зову / не зову / есть вопросы», ответ возвращается рекрутёру. Приватность встроена: ФИО кандидата видит рекрутёр, но в модель оно не уходит — ПДн вычищаются до обращения к ИИ.",
      benefit: "Замер HR-эксперта на реальной вакансии: ручной разбор откликов — это рабочий день, около 8 часов (пик доходил до 300 резюме за день — «вручную это ад»). С агентом: прогоняете пачку, дальше рекрутер смотрит только зелёную зону (в примере 8 кандидатов), внимательно и сразу пишет им, попутно берёт жёлтых — итого примерно 1,5 часа вместо 8. Механика простая: агент быстро находит идеальный фит, вы сразу ставите собесы зелёным, а пока ждёте собеседования — разбираете жёлтых. Плюс критерии найма перестают жить в голове рекрутера — их видно и можно править, и один настроенный агент работает на любую вакансию. А там, где вы переписываете его черновики приглашений и отказов, платформа замечает разницу и на этом его настраивает.",
    } },
  "Легал": { icon: ShieldCheck, color: "#7A6753", sub: "AI-юрист · договоры и риски", cat: "Право и договоры", soon: true,
    task: "Договоры и юр-вопросы упираются в одного юриста — и ждут в очереди",
    result: "Читает договор, называет риски простым языком, советует по переговорам и комплаенсу. Со своей приватной базой практики.",
    effect: "Больше контроля", speed: "С нашим спецом",
    tags: ["Договоры", "Риски", "Комплаенс", "База практики"],
    pitch: "Юрист прямо в переписке: разбирает присланный договор любого формата, называет риски и слабые пункты простым языком, подсказывает по переговорной позиции, комплаенсу и инцидентам. Опирается на вашу приватную базу практики — методички и документы, которые вы ему загрузили. Юридическая стратегия остаётся закрытой: переписка с ним не подмешивается в общую память компании. Пока советник — разбор и черновик вам на стол, решение за вами.",
    landing: {
      situation: "Договоры, оферты, ответы контрагентам, вопросы «а так вообще можно?» — всё стекается на одного юриста, а если своего нет, то на руководителя. Каждый документ надо вычитать целиком и держать в голове прошлую практику компании.",
      problem: "Юрист становится узким местом: пока он занят, договоры и ответы ждут днями. Разбор присланного договора — это вручную прочитать десяток страниц и не пропустить рискованный пункт. А наработанная практика — шаблоны, прошлые правки, позиции по спорным условиям — живёт в головах и разрозненных папках и каждый раз собирается заново.",
      solution: "Пришлите договор или вопрос прямо в переписку. Легал читает документ любого формата целиком, называет риски и слабые места простым языком и подсказывает по переговорной позиции, комплаенсу и инцидентам. Опирается на вашу приватную базу практики — методички и документы, которые вы ему загрузили. Юридическую стратегию держит закрытой: переписка с ним не утекает в общую память компании. Пока это советник — разбор и черновик кладёт вам на стол, решение принимаете вы.",
      benefit: "Первичный разбор договора и ответ на юр-вопрос — за минуты, а не в очереди к юристу. Риски видно сразу и на понятном языке, а практика компании перестаёт жить в голове одного человека — она в базе, к которой агент обращается каждый раз. Юрист занимается сложным и спорным, рутинную вычитку берёт на себя агент.",
    } },
...
const AGENT_CATS = {
  "Руководителю": c.violet, "Финансы и учёт": c.green, "Маркетинг": c.cyan, "Гостям и продажам": c.amber, "Найм и HR": c.blue, "Право и договоры": "#7A6753",
};
...
const DIMS = {
  role:   { label: "по отделу",   key: "cat",    values: ["Руководителю", "Финансы и учёт", "Маркетинг", "Гостям и продажам", "Найм и HR", "Право и договоры"] },
...
const clients = [
  ...
  { id: "c2", name: "ГК «Минутка»", seg: "Киоски · сеть" },
...
const libAssets = [
  ...
  ["промпт-скелет брифа", "Argus @ Минутка", "template", 2], ["движок voice-rules", "Hermes @ Вкус", "template", 2],
  ["модель объектов управления", "Kairos @ Минутка", "template", 2], ["прокси-набор ПДн", "платформа", "runtime", 5],
];
const CATALOG = [
...
function Catalog({ onOpenSol }) {
  ...
  const flag = ["Argus", "Kairos", "ИИ-Продакт", "Сводно", "Hermes", "Маркетинг-лид", "ИИ-хостес", "Recruiter", "Легал"].filter((s) => pick === "Все" || SOL[s][DIMS[dim].key] === pick);
  return (
    <div>
