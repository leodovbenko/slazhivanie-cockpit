...
import {
  ...
  CircleDot, ArrowRight, Bot, Search, Check, Send, Link2, UserCheck, Lightbulb, Users,
} from "lucide-react";
...
const SOL = {
  ...
  "Аутрич": { icon: Send, color: c.amber, sub: "Приём лидов, обогащение и черновик касания", cat: "Маркетинг",
    task: "Личное касание жрёт часы: под каждого найти повод, свести факты, написать не под копирку",
    result: "Приносишь контакты — обогащает кучей способов и готовит персональный посыл в твоём голосе. Отправляешь сам.",
    effect: "Меньше рутины", speed: "В пару кликов",
    tags: ["Приём лидов", "Обогащение", "Повод", "Черновик"],
    pitch: "Приносишь контакты как есть — список текстом, пересланный профиль, скриншот. Агент их принимает, чистит и склеивает дубли, а дальше обогащает каждого кучей способов сразу: читает его публичный Telegram-канал, ищет в вебе с защитой от однофамильцев, поднимает реестр по ИНН — и собирает досье со свежим датированным поводом. На этом досье пишет короткое персональное касание строго в твоём голосе и сверяет черновик на личное из твоей памяти. Готовый заход — тебе на стол, отправляешь сам со своего аккаунта.",
    landing: {
      situation: "Холодное касание работает, когда оно личное и по делу. Но подготовить его руками — найти под человека свежий повод, свести факты из разных источников, написать не под копирку в своём тоне — это часы. Поэтому либо не доходят руки, либо всё скатывается в шаблонную рассылку.",
      problem: "Персональные касания качественные, но их мало и они не масштабируются: на одного уходит уйма времени, а к концу дня тон плывёт от усталости. Найти актуальный повод под каждого, проверить, что это тот самый человек, а не однофамилец, собрать факты — всё вручную и медленно.",
      solution: "Приносишь контакты как есть — список, пересланный профиль, скриншот; агент принимает их, чистит и дедуплицирует. Дальше обогащает каждого сразу несколькими способами: читает его публичный Telegram-канал, ищет в вебе с защитой от однофамильцев, поднимает реестр по ИНН — и собирает досье со свежим датированным поводом и фактами, с указанием источников. На этом досье пишет короткое персональное касание строго в твоём голосе и перед показом сверяет черновик с памятью бизнеса: не проскочило ли внутреннее имя клиента или сделки. Готовый заход кладёт тебе на стол — отправляешь его ты сам, со своего аккаунта.",
      benefit: "Час ручной подготовки касания превращается в минуту проверки. Контакты, которые ты и так собираешь, приходят обогащёнными и с готовым персональным заходом в твоём голосе — тебе остаётся глянуть и отправить. Ничего не уходит само и мимо твоего решения, а внутренние имена не утекают наружу.",
    } },
  Recruiter: { icon: UserCheck, color: c.blue, sub: "AI-агент-рекрутер", cat: "Найм и HR", soon: true,
    devStatus: "Ядро — коннектор к hh.ru, оценка резюме по светофору, доска найма, калибровка на ваших исправлениях, открытый экран настройки промпта, лидерборд финалистов и оффер как отдельный шаг — уже работает на проде. Зона агента заканчивается на оффере: выход человека на работу он записывает как факт и считает в воронке, но онбординг и оформление не ведёт — столбцов под то, чего агент не делает, мы не рисуем. Живой демо-дашборд появится здесь, когда агент выйдет в кокпит.",
    ...
    tags: ["hh.ru", "Светофор", "Калибровка", "Воронка", "Оффер"],
    pitch: "Читает полный текст резюме — не только поля — и раскладывает кандидатов по трём зонам светофора с обоснованием и цитатами. Критерии не выдумывает: применяет те, что задал рекрутер под конкретный наём. Калибровка сверяет его оценки с вашими решениями, аналитика воронки показывает узкое место, а разбор собеса идёт по компетенциям вакансии. После собеседования агент сам спрашивает нанимающего менеджера, как оно прошло, и складывает его ответ в карточку кандидата — обратная связь больше не остаётся в голове. Тех, кого нанимающий одобрил после разговора, агент ставит рядом и сравнивает по одним осям — включая зарплатные ожидания против вашей вилки, потому что выбор между двумя сильными людьми делается по деньгам. Оффер — отдельный шаг с датой и суммой, если её называют, и у него два честных конца: человек вышел на работу или отказался, и причину записываем, если её назвали. Одно ядро настраивается под любую вакансию. И это не чёрный ящик: на экране настройки видно, из чего собирается оценка — ваше ТЗ, карта требований, наш каркас правил, ваша дописка — и можно посмотреть дословный текст, который уйдёт в модель, до прогона.",
    landing: {
      ...
      solution: "Агент читает полный текст каждого резюме и раскладывает по трём зонам светофора — с обоснованием и цитатами из резюме. Критерии не придумывает сам: применяет те, что рекрутер задал под конкретный наём. Передача нанимающему менеджеру — двусторонняя: он видит на отдельной странице, почему кандидат прошёл, и отвечает «зову / не зову / есть вопросы», ответ возвращается рекрутёру. Слово нанимающего нужно дважды, и второй раз агент спрашивает сам: через час после собеседования он приходит к менеджеру в Telegram — «разговор прошёл, что скажете по кандидату теперь?» — и принимает ответ кнопкой, текстом или голосом, а на его рабочем столе видно, по кому слово за ним. Дальше выбор: одобренных после разговора агент ставит рядом и сравнивает по одним осям — требования вакансии, слово нанимающего, зарплатные ожидания против вашей вилки. Сравнивает только внутри одной вакансии: требования у разных вакансий разные, и складывать их в общий рейтинг значило бы обманывать себя. Оффер — отдельный шаг с датой и суммой, если её называют, и у него два конца: человек вышел на работу или отказался. Причина отказа остаётся записанной, и самая дорогая потеря в найме попадает в воронку, а не растворяется. Предложение вы делаете сами: платформа кандидату ничего не пишет и текст оффера не придумывает, она записывает то, что уже произошло. Приватность встроена: ФИО кандидата видит рекрутёр, но в модель оно не уходит — ПДн вычищаются до обращения к ИИ. Как агент судит — открыто: отдельный экран показывает оценку слоями (ваше ТЗ → карта требований → наш каркас правил → ваша дописка → поправки, выученные на ваших исправлениях) и печатает дословный текст, который уйдёт в модель, — до того, как вы запустите прогон.",
  ...
  "Легал": { icon: ShieldCheck, color: "#7A6753", sub: "AI-юрист · договоры и риски", cat: "Право и договоры", soon: true,
    devStatus: "Ядро — разбор присланного договора и юр-вопросов прямо в переписке, с опорой на вашу приватную базу практики — уже работает. Коннектор к справочно-правовым системам (ПравоТех) — на дорожной карте. Живой демо-дашборд появится здесь, когда агент выйдет в кокпит.",
  ...
  "Комьюнити": { icon: Users, color: c.clay, sub: "AI комьюнити-менеджер · закрытый чат-сообщество", cat: "Сообщество", soon: true,
    devStatus: "Вечерний дайджест «вопрос → как решилось» по реальным тредам ответов — уже на проде, и в чат он уходит только после подтверждения человека. Там же на проде разбор сообщества для продукта: раз в неделю повторяющиеся боли, просьбы, обходные пути и вопросы копятся доской у продакта, а повторившееся или подтверждённое вами приносит в личку тому, кого вы назначили — не назначили, значит молчит. В чат этот разбор не пишет ничего. Дальше по дорожной карте — ответы на вопросы по чату, недельный топ благодарностей и активности, модерация. Живой демо-дашборд появится здесь, когда агент выйдет в кокпит.",
    task: "Ценные вопросы и ответы тонут в потоке чата-сообщества",
    result: "Каждый вечер — дайджест «вопрос → как решилось» на ваш Ок, и в чат после него. Ничего разобранного не теряется.",
    effect: "Меньше рутины", speed: "С нашим спецом",
    tags: ["Дайджест", "Знание", "Продукт-сигналы", "Активность", "Модерация"],
    pitch: "Читает закрытый чат-сообщество с атрибуцией и каждый вечер собирает дайджест дня — какие вопросы задали и как их решили, тредами по реальным ответам-reply, а не выдуманным связям — и приносит его человеку на подтверждение. В чат пост уходит только после нажатия «Отправить»; не ответили — не уходит. В тихий день молчит. По умолчанию раз в неделю отдельно разбирает переписку для продукта: что болит, о чём просят, как выкручиваются в обход и что спрашивают снова и снова — повторяющееся копится доской у продакта, а повторившееся или подтверждённое вами уходит в личку тому, кого вы назначили (не назначили — молчит). В чат этот разбор не пишет ни строки. Знание сообщества держит отдельной полкой, не растворяя в памяти компании. Дальше — Q&A по чату, счёт благодарностей и активности, модерация. Промпт дайджеста вы правите сами в кабинете.",
    landing: {
      situation: "У вас закрытый профессиональный чат-сообщество — клиентов, подписчиков или отрасли — на сотни, а то и тысячи человек. Сотни сообщений в день: вопросы, разборы, «а как вы решили вот это?».",
      problem: "Поток невозможно прочитать целиком. Ценные вопросы и ответы «как решить» тонут в ленте за час, новичок ищет то, что уже сто раз разбирали, а живой модератор один и выгорает. Знание сообщества нигде не оседает — оно растворяется в переписке.",
      solution: "Комьюнити-менеджер тихо читает чат с атрибуцией (кто что сказал) и каждый вечер собирает короткий дайджест дня: какие вопросы задали и как их решили. В чат он уходит только после того, как человек нажал «Отправить». Кнопки лежат в кабинете, где текст можно поправить перед отправкой, и приходят в Telegram тому, кого вы назначили утверждать; пока не назначили — дайджест просто ждёт в кабинете, никого не дёргая. Никто не ответил — дайджест не уходит вовсе. Единица — тред «вопрос → ответ», собранный по реальным ответам-reply (как нативный «просмотреть N ответов»), поэтому в дайджесте нет выдуманных пар и нет вопросов без ответа. В тихий день он молчит, а знание сообщества держит на отдельной полке, не смешивая тысячу чужих голосов с памятью вашей компании. Раз в неделю он же разбирает переписку для продукта — но это разбор внутрь, а не в чат: боли, просьбы, обходные пути, повторяющиеся вопросы и слова сообщества копятся на доске у продакта, и только повторившееся, а не каждая реплика. Подписи участников в этот разбор не уходят: агент видит текст без «кто сказал» — продукту нужен паттерн, а не автор. Если человек сам назвал имя внутри сообщения, оно останется в тексте: вычищать чужие реплики мы не беремся и не обещаем. Дальше по дорожной карте — ответы на вопросы по чату, недельный топ благодарностей и активности, модерация. Промпт дайджеста вы правите сами в кабинете.",
      benefit: "Участник видит выжимку ценного за минуту вместо прокрутки сотен сообщений, а ни один разобранный вопрос не теряется — знание сообщества накапливается и остаётся под рукой. Нагрузка на живого модератора падает. И это честно по устройству: дайджест собирает только то, на что реально ответили, и не выдаёт сегодняшний ответ за новый вопрос. Пока премодерация включена — а по умолчанию она включена, — ни одно слово не попадает вашим людям без человека: агент собирает, отправляете вы. Выключить её можете тоже вы. Продукт получает то, за чем обычно ходят на кастдевы: не поток и не пересказ дня, а список того, что в чате повторяется, со счётчиком — и счётчик честный, «замечен в 4 разборах» значит ровно это, а не «сказали 4 человека». Задачи по нему заводит человек: агент не двигает ваш бэклог сам. Отдельный бот заводить не нужно — добавьте в чат вашего платформенного бота, сделайте его админом и выберите в кабинете, кто утверждает дайджест: пока этого человека нет, черновик ждёт в кабинете и через 20 часов закрывается, так и не уйдя в чат.",
    } },
...
const AGENT_CATS = {
  "Руководителю": c.violet, "Финансы и учёт": c.green, "Маркетинг": c.cyan, "Гостям и продажам": c.amber, "Найм и HR": c.blue, "Право и договоры": "#7A6753", "Сообщество": c.clay,
};
...
const DIMS = {
  role:   { label: "по отделу",   key: "cat",    values: ["Руководителю", "Финансы и учёт", "Маркетинг", "Гостям и продажам", "Найм и HR", "Право и договоры", "Сообщество"] },
...
function Catalog({ onOpenSol }) {
  ...
  const flag = ["Argus", "Kairos", "ИИ-Продакт", "Сводно", "Hermes", "Маркетинг-лид", "ИИ-хостес", "Recruiter", "Легал", "Комьюнити"].filter((s) => pick === "Все" || SOL[s][DIMS[dim].key] === pick);
  return (
    <div>
    ...
    <div>
      ...
      {soon ? (s.devStatus ? (
        <Card style={{ borderLeft: `3px solid ${col}`, background: c.panelHi }}>
          ...
          <div style={{ fontSize: 14, lineHeight: 1.55, color: c.txt }}>{s.devStatus}</div>
        </Card>
      ) : null) : (<>
